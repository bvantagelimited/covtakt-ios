...
//    @IBOutlet weak var animationView: AnimationView!
    ...
    @IBAction func onDebugTapped(_ sender: Any) {
        self.performSegue(withIdentifier: "HomeToDebugSegue", sender: self)
        
    }
    @IBAction func onNotificationTab(_ sender: Any) {
    ...
    override func viewDidLoad() {
        ...
        button.setTitle(NSLocalizedString("Use power saver mode", comment: "Use power saver mode"), for: .normal)

        observeNotifications()
//        animationView.loopMode = LottieLoopMode.playOnce
        updateDevice()
        bleAuthorized = BluetraceManager.shared.isBluetoothAuthorized()
        if(bleAuthorized == false) {
            let alertController = UIAlertController(title: "", message: "Aktivirajte Bluetooth", preferredStyle: .alert)
            let settingsAction = UIAlertAction(title: "Podešavanja", style: .default) { (_) -> Void in
            ...
            let cancelAction = UIAlertAction(title: "Otkažite", style: .default, handler: nil)
    ...
    func updateDevice() {
        ...
        print("savedDeviceToken",savedDeviceToken)
        FirebaseAPIs.setDeviceToken(token: savedDeviceToken, {_ in
    ...
    func observeNotifications() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.alert(notif:)), name: NSNotification.Name.NotificationAlert, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(self.refreshToken), name: NSNotification.Name("RefreshFCMToken"), object: nil)
    }
    @objc func alert(notif: Notification) {
        if let title = notif.userInfo?["title"] as? String, let body = notif.userInfo?["body"] as? String {
            Alert.showMessage(self, title: title, message: body)
...
struct TracerRemoteConfig {
    ...
    static var defaultShareText = ""

    init() {
        // Setup remote config
        TracerRemoteConfig.defaultShareText = NSLocalizedString("Join me in stopping the spread of COVID-19! Download Covtakt!", comment: "")
        print(TracerRemoteConfig.defaultShareText)
